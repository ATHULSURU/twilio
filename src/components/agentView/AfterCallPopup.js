...
class AfterCallPopup extends React.Component{
    constructor(props) {
        ...
        this.state = {
            ...
            'Promise - payment in full',
                ...
                'Promise - One Time Payment',
                'commitment - payment in full',
                'commitment - payment plan',
                'commitment - settlement in full',
                'commitment - settlement in payments',
                'commitment - one time payment',
                'No pay - not right now',
                'No pay - never pay',
                'No pay - can\'t talk now',
                'No pay - cancel payment plan',
                'Bankruptcy'
            ],
    ...
    renderPossibleTags(){
        ...
        for (let i = 0; i < this.state.possibleTags.length; i++) {
            ...
            tags.push(
                <Tag key={`possible_${tag}`} disabled={(this.state.selectedTags.includes(tag))} onClick={()=>{this.addTag(tag)}}>{tag}</Tag>
            )
        }
        return tags;
    }
